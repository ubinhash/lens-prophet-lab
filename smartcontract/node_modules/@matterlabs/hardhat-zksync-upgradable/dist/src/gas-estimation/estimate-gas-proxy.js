...
function makeEstimateGasProxy(hre) {
    return async function estimateGasProxy(deployer, artifact, args = [], opts = {}, quiet = false) {
        ...
        switch (kind) {
            ...
            case 'transparent': {
                const { adminGasCost, proxyGasCost } = await estimateGasTransparent(hre, deployer, mockImplAddress, data, initialOwner, quiet);
...
async function estimateGasUUPS(hre, deployer, mockImplAddress, data, quiet = false) {
    const proxyContract = await (0, factories_1.getProxyArtifact)(hre);
    try {
...
async function estimateGasTransparent(hre, deployer, mockImplAddress, data, initialOwner, quiet = false) {
    const adminArtifact = await (0, factories_1.getProxyAdminArtifact)(hre);
    ...
    const TUPContract = await (0, factories_1.getTransparentUpgradeableProxyArtifact)(hre);
    try {
